...
export class Menu {
  ...
  public async showMenu(
    ...
    const { level, headline } = Object.assign(
      {
        ...
        headline: parentCommands
          ? parentCommands.charAt(0).toUpperCase() +
            parentCommands.slice(1) +
            ' commands'
          : '',
      },
      options
    ...
    let mainCommands = commands
      ...
      .map(
        (c) =>
          c.commandPath[level] + (c.description ? ` (${c.description})` : '')
      )
      .sort()
    ...
    switch (commandName) {
      ...
      case messages.help: {
        ;(print.printCommands as any)(
          this.toolbox,
          level ? parentCommands.split(' ') : undefined
        )
        break
      }
      default: {
        ...
        const command = commands.filter(
          (c: any) =>
            c.commandPath.join(' ') ===
            `${parentCommands} ${commandName}`.trim().replace(/\s\(.*\)$/, '')
        )[0]
